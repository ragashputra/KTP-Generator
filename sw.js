const CACHE_NAME = 'cetak-ktp-v54'; // UPDATE v54: header kemarin sudah dapat background yg jelas (v53) tapi masih terasa flat/polos -- ditambah motif dekoratif gelombang (wave) halus di dasar header, digambar pakai CSS mask dari SVG wave inline (bukan file gambar terpisah), di-fill var(--accent) opacity sangat rendah (10%) supaya SUBTLE/elegan, tidak mengganggu keterbacaan teks brand di atasnya. Warnanya OTOMATIS ikut berubah kalau user ganti tema warna aksen lewat menu Pengaturan (krn pakai var(--accent), bukan warna hardcode) -- jadi tetap konsisten & "hidup" mengikuti tema pilihan, baik di dark maupun light mode. header.top diberi overflow:hidden supaya wave-nya rapi terkurung mengikuti bentuk header, tidak memengaruhi tinggi/layout header sama sekali (murni dekorasi ::after, position absolute).
