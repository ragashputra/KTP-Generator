const CACHE_NAME = 'cetak-ktp-v53'; // UPDATE v53: header (topbar) sebelumnya nyaris polos tanpa background yg jelas -- token --header-bg lama warnanya nyaris identik dgn --bg (warna body), cuma beda opacity dikit, jadi keliatan nge-blend/nyatu total ke konten di bawahnya. Diganti jadi gradient halus berbasis --surface (bukan --bg) + shadow lembut di bawah header (--header-shadow, kesan "melayang" di atas konten) + border-bottom yg lebih tegas (--header-border) -- header sekarang punya lapisan warna sendiri yg jelas terpisah dari body, terasa premium & profesional, tetap pakai backdrop-blur (glass) spy konsisten sama estetika bottom navbar. Berlaku di dark & light mode via token terpisah spt biasa.
